...
const DgaBreadcrumbs = class {
    ...
    getArrowIcon() {
        return dir.isRTL(this.el) ? index$1.arrowLeft01StrokeRoundedIcon : index$1.arrowRight01StrokeRoundedIcon;
    }
    render() {
        const { el } = this;
        const dir$1 = dir.isRTL(el) ? 'rtl' : 'ltr';
        return (index.h(index.Host, { key: 'fb59028d99df07ab8d9e39de982f43d9f4404b5e', id: this.componentId, style: this.rootStyles, dir: dir$1 }, index.h("nav", { key: '1a3e10885e2a76ce6052400118f2a8c84049e476', "aria-label": "breadcrumb" }, index.h("ul", { key: '42b42572022ec7e3ed8ecad3474191307df51d9f', class: `dga-breadcrumb dga-breadcrumb--${dir$1}` }, this.breadcrumbItems.map((item, index$1) => {
            if (this.isEllipsisItem(item)) {
                return (index.h("li", { key: index$1, class: "dga-breadcrumb-item ellipsis" }, index.h("span", { class: "dga-breadcrumb-icon" }, index.h("dga-icon", { icon: this.getArrowIcon() })), index.h("a", { onClick: () => {
                        this.toggleMenu();
                        return false;
                    }, "aria-haspopup": "true", "aria-expanded": this.isMenuOpen ? 'true' : 'false', class: "link link--md link--neutral" }, "..."), this.isMenuOpen && (index.h("ul", { class: "dga-breadcrumb-dropdown" }, item.items.map(ellipsedItem => (index.h("li", { key: ellipsedItem === null || ellipsedItem === void 0 ? void 0 : ellipsedItem.label, class: "dga-breadcrumb-dropdown-item" }, index.h("a", { onClick: e => this.handleClick(e, ellipsedItem.path, ellipsedItem), class: {
                        'disabled-link': ellipsedItem.disabled,
                    }, href: ellipsedItem.disabled ? undefined : ellipsedItem.path, "aria-disabled": ellipsedItem.disabled ? 'true' : 'false' }, index.h("span", { class: "link__label" }, ellipsedItem.label)))))))));
            }
            const isLastItem = index$1 === this.breadcrumbItems.length - 1;
            const first = index$1 === 0;
            return (index.h("li", { key: index$1, class: {
                    'dga-breadcrumb-item': true,
                    'active': isLastItem,
                }, "aria-current": isLastItem ? 'page' : undefined }, item.path && !isLastItem ? (index.h(index.Fragment, null, !first && (index.h("span", { class: "dga-breadcrumb-icon" }, index.h("dga-icon", { icon: this.getArrowIcon() }))), index.h("a", { onClick: e => this.handleClick(e, item.path, item), class: {
                    'link link--md link--neutral': true,
                    'disabled-link': item.disabled,
                }, href: item.disabled ? undefined : item.path, "aria-disabled": item.disabled ? 'true' : 'false' }, index.h("span", { class: "link__label" }, item.label)))) : (index.h(index.Fragment, null, index.h("span", { class: "dga-breadcrumb-icon" }, index.h("dga-icon", { icon: this.getArrowIcon() })), index.h("a", { onClick: e => this.handleClick(e, item.path, item), class: {
                    'link link--md link--neutral': true,
                    'disabled-link': item.disabled,
                    'link_empty': true,
                }, href: item.disabled ? undefined : undefined, "aria-disabled": item.disabled ? 'true' : 'false' }, index.h("span", { class: "link__label" }, item.label))))));
        })))));
    }
...
DgaBreadcrumbs.style = dgaBreadcrumbsCss;

exports.dga_breadcrumbs = DgaBreadcrumbs;
