...
const DgaRating = class {
    ...
    render() {
        return (index.h(index.Host, { key: 'cd38dc21ae1deeddce0086424231dfeb30329ed3' }, index.h("span", { key: 'c7ae687c27cd5ea89480a653edf27ea035a5b25b', class: `dga-rating dga-rating--${this.size}`, ref: el => (this.rootRef = el), onMouseMove: (event) => this.handleMouseMove(event), onMouseLeave: (event) => this.handleClear(event), dir: rtlUtils.isElementRTL(this.el) ? 'rtl' : 'ltr' }, Array.from({ length: this.max }, (_, index) => index + 1).map(value => {
            const colorMapper = {
                default: 'var(--background-secondary)',
                brand: 'var(--background-primary)',
            };
            // Detect RTL for half-star positioning
            const isRTL = rtlUtils.isElementRTL(this.el) ? 'rtl' : 'ltr';
